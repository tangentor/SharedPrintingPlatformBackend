...
public class SharefileServiceImpl extends ServiceImpl<SharefileMapper, Sharefile>
    ...
    public List<Sharefile> handleURLtoReal(List<Sharefile> list) {
        list.forEach(file->{
            file.setThumbnail(file.getThumbnail().replace("$ip", IPUtil.getLocalIpAddress()));
            file.setUrl(file.getUrl().replace("$ip", IPUtil.getLocalIpAddress()));
            //处理用户
            LambdaQueryWrapper<User> userWrapper = new LambdaQueryWrapper<>();
            userWrapper.select(User::getNickname);
            userWrapper.eq(User::getUsername,file.getUid());
